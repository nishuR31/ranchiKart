...
export function assertSize(product: Product, width?: number, height?: number) {
  ...
  if (width < product.minWidthMm || width > product.maxWidthMm) throw new Error(`${product.name} width is out of range`);
  if (height < product.minHeightMm || height > product.maxHeightMm) throw new Error(`${product.name} height is out of range`);
}
...
export function unitPrice(product: Product, variant?: ProductVariant | null, width?: number, height?: number) {
